...
A user with administrative privileges (i.e. role set to `ADMIN`) can log into the Ego [Admin UI](/documentation/ego/user-guide/admin-ui) and perform all admin operations without needing to interact directly with the API.

However, there may often be cases where a user needs or wants to interact with the API endpoints directly over a command line interface (by issuing cURL commands) or using the Swagger UI.  In these scenarios, the user will need to authenticate with the 
